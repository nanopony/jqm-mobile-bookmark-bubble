...
google.bookmarkbubble.Bubble.prototype.OPERA_USERAGENT_REGEX_ = /OPR/;
google.bookmarkbubble.Bubble.prototype.FIREFOX_USERAGENT_REGEX_ = /Firefox/;
/**
 ...
 */
 ...
 google.bookmarkbubble.Bubble.prototype.isOpera_ = function() {
 	return this.OPERA_USERAGENT_REGEX_.test(window.navigator.userAgent);
 };

 google.bookmarkbubble.Bubble.prototype.isFirefox_ = function() {
 	return this.FIREFOX_USERAGENT_REGEX_.test(window.navigator.userAgent);
 };
 ...
 */
 ...
 google.bookmarkbubble.Bubble.prototype.isMobile_ = function() {
 	return (window.navigator.userAgent.indexOf("Mobile Safari") >= 0) || (window.navigator.userAgent.indexOf("Mobile") >= 0);
 };
 ...
 */
 google.bookmarkbubble.Bubble.prototype.getVisibleYPosition_ = function() {
 	return this.bubbleConfig.verticalLocation === this.consts.verticalLocation.VERTICAL_TOP ?
 ...
 */
 google.bookmarkbubble.Bubble.prototype.getLink = function(rel) {
	...
	/** Sets platfrom-specific text 
	*/

	google.bookmarkbubble.Bubble.prototype.consts = {
		horizontalLocation: {HORIZONTAL_RIGHT: 1, HORIZONTAL_CENTER: 2, HORIZONTAL_LEFT: 3},
		verticalLocation: {VERTICAL_TOP: 1, VERTICAL_BOTTOM: 2},
		bubbleBackground: {BLUE_GRADIENT_DIRECT: 1, BLUE_GRADIENT_INVERSE: 2}
	}
	google.bookmarkbubble.Bubble.prototype.applyStyle_ = function(obj, styles) {
		for (var style_key in styles) {
			obj.style[style_key] = styles[style_key];
		}
	}

	google.bookmarkbubble.Bubble.prototype.getBubbleConfig = function() {
		var config = {
			message : '',
			style: {},
			horizontalLocation: this.consts.horizontalLocation.HORIZONTAL_CENTER,
		...
		if (isAndroid) {
			config.style.font = '0.625em sans-serif';

			if (this.isOpera_() && this.isMobile_()) {
				config.horizontalLocation = this.consts.horizontalLocation.HORIZONTAL_LEFT;
				config.verticalLocation = this.consts.verticalLocation.VERTICAL_TOP;
				config.message = this.msg.opera;
				return config;
			}	
			if (this.isFirefox_() && this.isMobile_()) {  			
				config.horizontalLocation = this.consts.horizontalLocation.HORIZONTAL_RIGHT;
				config.verticalLocation = this.consts.verticalLocation.VERTICAL_TOP;
				config.message = this.msg.firefox;
				return config;
			}
  		// Google Chrome // WebView
  		if (this.getAndroidVersion_() < this.getVersion_(3, 0)) {
  			config.message = this.msg.android;
  		}
  		else {
  			config.horizontalLocation = this.consts.horizontalLocation.HORIZONTAL_RIGHT;
  			config.verticalLocation = this.consts.verticalLocation.VERTICAL_TOP;
  			config.bubbleBackground = this.consts.bubbleBackground.BLUE_GRADIENT_DIRECT;

  			if ((this.getAndroidVersion_() < this.getVersion_(4, 0)) && this.isMobile_()) {
  				config.message = this.msg.android3;
  			}
  			else {
  				if ((this.getAndroidVersion_() >= this.getVersion_(4, 4)) && this.isMobile_()) {
  					config.message = this.msg.android44;
  				}
  				else {
  					if ((this.getAndroidVersion_() < this.getVersion_(4, 1)) && this.isMobile_()) {
  						config.message = this.msg.android4;
  					}
  					else {
  						config.message = this.msg.android41;
  					}
  				}
  			}
  		}
  		...
  		config.message = this.msg.blackberry;
  		return config;
  	}
  	else if(isPlayBook) {
  		config.bubbleBackground = this.consts.bubbleBackground.BLUE_GRADIENT_DIRECT;
  		config.verticalLocation = this.consts.verticalLocation.VERTICAL_TOP;
  		config.message = this.msg.playbook;
  		...
  		if (this.getIosVersion_() >= this.getVersion_(7, 0)) {
  			config.message = this.msg.ios7orlater;
  		}
  		else if (this.getIosVersion_() >= this.getVersion_(4, 2)) {
  			config.message = this.msg.ios42orlater;
  		}
  		else {
  			config.message = this.msg.ioslegacy;
  		}
  		return config;
  	}
 ...
 */
 google.bookmarkbubble.Bubble.prototype.build_ = function() {
 	...
 	var config  = this.getBubbleConfig();
 	...
 	switch (config.horizontalLocation) {
 		case this.consts.horizontalLocation.HORIZONTAL_RIGHT:
 		bubbleInner.style.margin = '0 0 0 ' +(window.innerWidth - 240) + 'px';
 		break;
 		case this.consts.horizontalLocation.HORIZONTAL_CENTER:
 		bubbleInner.style.margin = '0 auto';
 		break;
 		case this.consts.horizontalLocation.HORIZONTAL_LEFT:
 		bubbleInner.style.margin = '0 0 0 82px' 
 		break;
 	}
	...
	bubbleInner.style.borderRadius = '8px';
 	...
 	bubbleInner.innerHTML = config.message;
	this.applyStyle_(bubbleInner, config.style); // custom style override
	...
	icon.style.background = '#fff url(' + this.getIconUrl_() + ') no-repeat 0 0';
